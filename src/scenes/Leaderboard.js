...
const STYLE_CONFIG = {
  ...
  color: {
    ...
    exportBtn: '#ffffffff',      // 导出按钮文字色
    exportBtnHover: '#b0b2b3ff', // 导出按钮悬浮色
    ...
    infoText: 'rgba(14, 248, 213, 1)',          // 图片信息文字色
    timeText: '#ffd700',       // 时间文字色
    emptyText: 'rgba(14, 248, 213, 1)'       // 空记录文字色
  },
  // 按钮样式：统一尺寸、内边距、圆角
  button: {
    height: 36,
    ...
    bg: {
      ...
      backHover: '#6fbbfeff',
      export: '#6fbbfeb9',
      exportHover: '#6fbbfeff',
      import: '#6fbbfeb9',
      importHover: '#6fbbfeff'
    }
  },
  // 排行榜板块样式
  section: {
...
export class Leaderboard extends Phaser.Scene {
  ...
  preload() {
    this.load.image('leaderboard_bg', './assets/1.jpg');
    // 预加载音频资源（避免播放时卡顿）
    const audios = [
  ...
  create() {
    ...
    this.add.image(centerX, H / 2, 'leaderboard_bg')
    .setDisplaySize(W, H)
  ...
  bg.fillStyle(0x0e1218, 0.2); // 降低不透明度，让背景图片能透出来
  bg.fillRect(0, 0, W, H);
  ...
  drawSection(x, y, width, title, titleColor, records) {
    ...
    sectionBg.fillStyle(0x6fbbfe, 0.4);
